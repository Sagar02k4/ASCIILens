// ============================================================
// AsciiLens — MobileBottomSheet Component (AF-098, AF-107)
// Bottom sheet for mobile controls
// ============================================================

import React, { useState } from 'react';
import './MobileBottomSheet.css';

export default function MobileBottomSheet({ children }) {
  const [isOpen, setIsOpen] = useState(false);

  return (
    <>
      {/* Bottom sheet toggle */}
      <button
        className="mobile-sheet-toggle"
        onClick={() => setIsOpen(!isOpen)}
        id="mobile-sheet-toggle"
        aria-label="Toggle controls"
      >
        <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
          <line x1="4" y1="21" x2="4" y2="14" />
          <line x1="4" y1="10" x2="4" y2="3" />
          <line x1="12" y1="21" x2="12" y2="12" />
          <line x1="12" y1="8" x2="12" y2="3" />
          <line x1="20" y1="21" x2="20" y2="16" />
          <line x1="20" y1="12" x2="20" y2="3" />
          <line x1="1" y1="14" x2="7" y2="14" />
          <line x1="9" y1="8" x2="15" y2="8" />
          <line x1="17" y1="16" x2="23" y2="16" />
        </svg>
      </button>

      {/* Bottom sheet */}
      {isOpen && (
        <div className="mobile-sheet-backdrop" onClick={() => setIsOpen(false)}>
          <div
            className="mobile-sheet"
            onClick={e => e.stopPropagation()}
            id="mobile-bottom-sheet"
          >
            <div className="mobile-sheet-handle" onClick={() => setIsOpen(false)}>
              <div className="mobile-sheet-handle-bar" />
            </div>
            <div className="mobile-sheet-content">
              {children}
            </div>
          </div>
        </div>
      )}
    </>
  );
}
